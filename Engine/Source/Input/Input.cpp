...
void Input::ParseInputData(UINT message, WPARAM wparam, LPARAM lparam)
{
	...
	switch (message)
	{
	case WM_KEYDOWN: if ((HIWORD(lparam) & KF_REPEAT) == 0) { Input::m_keyDownArray[key_code] = true; } break;//直前のキー状態を参照し 瞬間か判定
	case WM_KEYUP: Input::m_keyUpArray[key_code] = true; break;
