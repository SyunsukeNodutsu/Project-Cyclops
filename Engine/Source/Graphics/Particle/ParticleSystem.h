...
public:
	...
	void Update(ID3D11ComputeShader* simulationShader);
	void Draw();

	void Emit(UINT particleMax, ParticleSystem::EmitData data, bool loop = false);
	void End();
...
private:
	...
	void SetupViews();//ビューはthreadsafeではないのでデバイス作成のスレッドで行う
...
public:
	...
	void Emit(EmitData data, bool loop = false);
	void AllEnd();
